...
import com.kirinyt.app.database.DBManager.SORTING
import com.kirinyt.app.database.dao.CommandTemplateDao
import com.kirinyt.app.database.dao.ResultDao
...
import com.kirinyt.app.database.repository.HistoryRepository.HistorySortType
...
import com.kirinyt.app.util.Extensions.isYoutubeChannelURL
...
//        }
    ...
    private suspend fun getYoutubeVideo(inputQuery: String, resetResults: Boolean, addToResults: Boolean) : List<ResultItem>{
        ...
        if (resetResults) {
            ...
            res.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = KIRINYT_SEARCH }
        }
        if (addToResults){
            val ids = resultDao.insertMultiple(res)
            ids.forEachIndexed { index, id ->
    ...
    private suspend fun getFromYTDLP(inputQuery: String, resetResults: Boolean, addToResults: Boolean, singleItem: Boolean = false) : List<ResultItem> {
        ...
        ytdlpUtil.getFromYTDL(inputQuery, singleItem) { results ->
            ...
            results.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = KIRINYT_SEARCH }
            if (addToResults) {
    ...
    suspend fun updateDownloadItem(
        ...
        if (downloadItem.needsDataUpdating()){
            runCatching {
                ...
                if (downloadItem.playlistTitle.isEmpty() || (downloadItem.playlistTitle.isNotBlank() && downloadItem.playlistTitle != KIRINYT_SEARCH)   ) downloadItem.playlistTitle = info.playlistTitle
