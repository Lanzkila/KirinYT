...
import com.kirinyt.app.core.packages.PackageBase.Companion.sharedClient
...
class UpdateUtil(var context: Context) {
    ...
    fun getGithubReleases(): List<GithubRelease> {
        val repository = BuildConfig.KIRINYT_UPDATE_REPOSITORY.trim()
        if (repository.isBlank()) {
            Log.w(tag, "KirinYT update repository is not configured")
            return emptyList()
        }
        val url = "https://api.github.com/repos/$repository/releases"
